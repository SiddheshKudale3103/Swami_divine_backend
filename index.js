...
// Ensure data/media directories exist
...
app.get("/api/images", (req, res) => {
  const manifest = readJSON(MEDIA_FILE);
  const base = `${req.protocol}://${req.get("host")}`;
  const images = (manifest.images || []).map((i) => ({
    ...i,
    url: i.url || base + i.src,
  }));
  res.json(images);
});
...
app.get("/api/videos", (req, res) => {
  const manifest = readJSON(MEDIA_FILE);
  const base = `${req.protocol}://${req.get("host")}`;
  const videos = (manifest.videos || []).map((i) => ({
    ...i,
    url: i.url || base + i.src,
  }));
  res.json(videos);
...
app.get("/api/pdfs", (req, res) => {
  const manifest = readJSON(MEDIA_FILE);
  const base = `${req.protocol}://${req.get("host")}`;
  const pdfs = (manifest.pdfs || []).map((i) => ({
    ...i,
    url: i.url || base + i.src,
  }));
  res.json(pdfs);
